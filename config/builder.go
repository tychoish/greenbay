package config

import (
	"encoding/json"
	"sync"

	"github.com/mongodb/greenbay"
	"github.com/pkg/errors"
)

// Builder provides an interface to build a GreenbayTestConfig
// object programatically. Primarily useful in testing and other options with
type Builder struct {
	conf  GreenbayTestConfig
	mutex sync.RWMutex
}

// NewBuilder constructs a fully initialized Builder object.
func NewBuilder() *Builder {
	c := &Builder{
		conf: *newTestConfig(),
	}

	return c
}

// Conf returns a fully constructed and resolved GreenbayTestConfig,
// returning an error if there ere any parsing or validation
// errors.
//
// Be aware that the config object returned is a *copy* of the config
// object in the builder, which allows you to generate and mutate
// generated multiple configs with impacting the builder's
// config. However, this operation does the correct but peculiar
// operation of copying a mutex in the config object.
//
// Additionally, in most cases you'll want a
// *pointer* to this object produced by this method.
func (b *Builder) Conf() (GreenbayTestConfig, error) {
	b.mutex.Lock()
	defer b.mutex.Unlock()

	b.conf.reset()
	if err := b.conf.parseTests(); err != nil {
		return GreenbayTestConfig{}, errors.Wrap(err, "problem refreshing config builder")
	}

	return b.conf, nil
}

// AddCheck takes a check object and adds it to the underlying config
// object.
func (b *Builder) AddCheck(check greenbay.Checker) error {
	if check == nil {
		return errors.New("cannot add nil ")
	}

	// build the check structure
	t := rawTest{
		Name:      check.ID(),
		Suites:    check.Suites(),
		Operation: check.Name(),
	}

	raw, err := json.Marshal(check)
	if err != nil {
		return err
	}

	t.RawArgs = raw

	// take the lock and modify the structure.
	b.mutex.Lock()
	defer b.mutex.Unlock()
	b.conf.RawTests = append(b.conf.RawTests, t)

	return nil
}

// Len returns the number of checks represented in the builder config
// object.
func (b *Builder) Len() int {
	b.mutex.RLock()
	defer b.mutex.RUnlock()

	return len(b.conf.RawTests)
}
